...
//only required to parse post request body payload
//app.use(bodyParser.json());
//app.use(bodyParser.urlencoded({ extended: true }));

// ETH BLOCKCHAIN CALL, READONLY, FREE CALL
...
// ETH BLOCKCHAIN CALL, CHANGE, NEED GAS, NOT FREE
...
// ETH BLOCKCHAIN CALL, READONLY, FREE CALL
app.route('/supplyRatePerBlock/ceth/').get((req, res) => {
  cEthContract.methods.supplyRatePerBlock().call()
    .then((result) => {
      const supplyRate = result / 1e18;
      return res.send(""+supplyRate);
    }).catch((error) => {
      console.error('[supplyRatePerBlock][ceth] error:', error);
      return res.sendStatus(400);
    });
});
